...
class PosViewModel(
    ...
    companion object { private const val TAG = "PosViewModel" }
    ...
    fun onCurrencyChange(newValue: String) {
        if (newValue.length <= 3) {
            _uiState.value = _uiState.value.copy(currency = newValue.uppercase(), errorMessage = null)
        }
    }
    ...
    fun generatePaymentOfferQr() {
        ...
        if (currencyCode.length != 3) {
            _uiState.value = _uiState.value.copy(errorMessage = "Invalid currency code (must be 3 letters)")
            return
        }

        _uiState.value = _uiState.value.copy(isLoading = true, errorMessage = null, statusMessage = "Generating offer...", qrCodeBitmap = null) // Clear old QR
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (qrBitmap != null) {
                        ...
                        _uiState.value = _uiState.value.copy(isLoading = false, errorMessage = "Failed to generate QR code", statusMessage = "Error")
                    }

                } else {
                    val errorBody = response.errorBody()?.string() ?: "Unknown API error"
                    Log.e(TAG, "API Error: ${response.code()} - $errorBody")
                    _uiState.value = _uiState.value.copy(isLoading = false, errorMessage = "API Error: ${response.code()}", statusMessage = "Error")                }

            } catch (e: Exception) {
                Log.e(TAG, "Network or other error", e)
                _uiState.value = _uiState.value.copy(isLoading = false, errorMessage = "Error: ${e.message}", statusMessage = "Error")            }
        }
    }
