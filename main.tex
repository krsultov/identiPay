...
Crucially, the UIO is \emph{rail-agnostic}. The protocol defines how the agreement is negotiated and signed, but does not strictly dictate the underlying pipes used to move the liquidity. identiPay allows the same identity and authorization flow to control disparate settlement engines.
...
Interoperability relies on a custom DID method with the following grammar:
\begin{center}
\texttt{did:identipay:<hostname>:<transaction-id>}
\end{center}
The \texttt{hostname} component identifies the merchant's resolution endpoint (a DNS name matching a Trust Registry entry); the \texttt{transaction-id} is an opaque identifier for a specific commerce proposal. Because proposals are too large for QR codes, the DID serves as a compact routing pointer. When a user scans a QR code, the wallet performs a deterministic resolution via an HTTPS GET request to:
\begin{center}
\texttt{https://<hostname>/api/identipay/v1/intents/<transaction-id>}
\end{center}

\paragraph{Trust Registry verification.}
Before signing any intent, the wallet cross-references the \texttt{hostname} against a decentralized \textbf{Trust Registry} maintained on the Sui blockchain. Each merchant entry in the Trust Registry contains a verified hostname, the merchant's DID~\cite{did-core}, and the merchant's X25519 public key (used for artifact encryption). Only proposals served from a hostname that matches a registered Trust Registry entry are presented to the user; unregistered or mismatched hostnames cause the wallet to reject the proposal before any signing occurs.
...
identiPay introduces a \textbf{stealth meta-address} system~\cite{buterin-stealth,eip5564}: a human-readable name (e.g., \texttt{@krum.idpay}) resolves to a pair of cryptographic public keys, \emph{never} to a blockchain address. Every incoming payment or commerce artifact is delivered to a fresh, one-time \emph{stealth address} that only the recipient can detect and spend from. No two transactions share an address. No on-chain address is ever publicly linked to the name.

\subsection{Identity Ingestion and Meta-Address Generation}
\paragraph{Credential ingestion.}
The wallet ingests a credential via NFC (national eID/passport~\cite{icao9303}), QR-mediated issuance, or secure retrieval of an EUDI-aligned Verifiable Credential~\cite{vc-data-model,eidas}. identiPay assumes an identity issuance stack consistent with the OpenID for Verifiable Credential Issuance/Presentation (OpenID4VC family)~\cite{oid4vci,oid4vp}.
...
\paragraph{Curve compatibility.}
Ed25519 operates on the twisted Edwards form of Curve25519~\cite{ed25519}, while X25519 uses the Montgomery form~\cite{curve25519}. The two representations are birationally equivalent: any point on one curve can be mapped to the other via a coordinate transformation. In practice, the wallet converts the X25519 viewing public key $K_{\mathrm{view}}$ to Edwards form before performing point arithmetic, and converts back when invoking X25519-specific routines. All scalar arithmetic in the stealth address protocol is performed modulo $\ell \approx 2^{252}$, the order of the Ed25519 base point $G$.

\paragraph{Identity commitment and anti-Sybil nullifier.}
The wallet computes two values from the credential fields, where $H$ denotes SHA-256:
\begin{enumerate}
  \item A \emph{salted commitment} for privacy:
  $$
  C = \mathrm{Poseidon}\big(\mathrm{IssuerCertHash},\; H(\mathrm{DocNumber}),\; H(\mathrm{DOB}),\; \mathrm{Salt}\big).
  $$
  The user-chosen salt ensures that the same credential produces distinct commitments across different protocols, preventing cross-platform correlation.

  \item A \emph{deterministic nullifier} for Sybil resistance:
  $$
  N = \mathrm{Poseidon}\big(\mathrm{IssuerCertHash},\; H(\mathrm{DocNumber}),\; H(\mathrm{DOB})\big).
  $$
  Because $N$ contains no user-chosen randomness, the same credential always produces the same nullifier regardless of salt choice.
\end{enumerate}

\paragraph{ZK proof of credential validity.}
Locally, the wallet generates a Groth16~\cite{groth16} proof attesting:
$$
\pi_{\mathrm{reg}} \leftarrow \mathrm{Prove}\big(\mathrm{CredentialValid} \land \mathrm{Commitment} = C \land \mathrm{Nullifier} = N\big).
$$
...
\begin{itemize}
  ...
  \item the identity commitment $C$,
  \item the anti-Sybil nullifier $N$, and
  \item the Groth16 proof $\pi_{\mathrm{reg}}$.
\end{itemize}
The on-chain registry enforces \emph{nullifier uniqueness}: since $N$ is deterministic (unsalted), the same government credential always produces the same nullifier and can register exactly one name, preventing Sybil attacks. The salted commitment $C$ provides cross-protocol privacy, while $N$ provides uniqueness within identiPay. The registry stores \textbf{public keys only}---no Sui address is recorded.
...
\begin{enumerate}
  ...
  \item Derive a scalar: $s = \mathrm{SHA256}(\mathrm{shared} \parallel \texttt{"identipay-stealth-v1"}) \bmod \ell$, where $\ell$ is the order of the Ed25519 base point.
  \item Compute the stealth public key: $K_{\mathrm{stealth}} = K_{\mathrm{spend}} + s \cdot G$.
  \item Derive the stealth Sui address~\cite{blake2}: $\mathrm{addr} = \mathrm{BLAKE2b\text{-}256}(0\texttt{x00} \parallel K_{\mathrm{stealth}})$.
...
\begin{enumerate}
  ...
  \item Compare the first byte of $\mathrm{shared}$ with the announced view tag. If they differ, discard this announcement immediately---this eliminates $\sim$255 out of 256 candidates before any further derivation.
  ...
  \item If matched, derive the stealth private key: $k_{\mathrm{stealth}} = k_{\mathrm{spend}} + s \pmod{\ell}$.
\end{enumerate}
The ECDH scalar multiplication (step~1) is performed for every announcement, but the view tag check (step~2) avoids the costlier downstream steps (scalar derivation, point addition, and address hashing) for $\sim$255/256 of them.
...
Alice's wallet, running in the background, scans each \texttt{StealthAnnouncement}. For each, it computes $\mathrm{shared} = k_{\mathrm{view}} \cdot R$ and checks the one-byte view tag---a fast comparison that eliminates $\sim$255 out of 256 announcements before the costlier address derivation steps. Because $k_{\mathrm{view}} \cdot R = k_{\mathrm{view}} \cdot r \cdot G = r \cdot K_{\mathrm{view}}$, Alice recovers the same shared secret Bob used. She derives the stealth address, confirms it matches, and computes the stealth private key $k_{\mathrm{stealth}} = k_{\mathrm{spend}} + s \pmod{\ell}$. She can now spend the 50~USDC whenever she chooses.
...
\pi_{\mathrm{pool}} \leftarrow \mathrm{Prove}\Big(
  ...
  \mathrm{note.amount} = W + W_{\mathrm{change}}
  \;\land\;
  \mathrm{nullifier} = \mathrm{Poseidon}(c, \mathrm{ownerKey})
  \;\land\;
  c_{\mathrm{change}} = \mathrm{Poseidon}(W_{\mathrm{change}}, \mathrm{ownerKey}, \mathrm{salt'})
\Big)
$$
where $W$ is the withdrawal amount and $W_{\mathrm{change}}$ is the change returned as a new note commitment $c_{\mathrm{change}}$. The proof reveals the Merkle root, the nullifier (preventing double-spend), the recipient address, the withdrawal amount, and the change commitment. It does \emph{not} reveal which note was spent or who owns it. The contract verifies the proof via \texttt{sui::groth16}, checks the nullifier against a spent-set, transfers $W$ to the recipient, and (if change exists) inserts a new note commitment for the remainder.
...
A merchant constructs a structured proposal (canonicalized JSON-LD~\cite{jsonld} or equivalent typed data), containing at minimum:
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
Field & Semantics \\
...
\texttt{merchant\_did} & merchant identity (DID~\cite{did-core}, certificate chain, or on-chain object) \\
...
Sui's object-centric model~\cite{sui} and programmable transaction blocks enable multi-step, multi-object transactions that either succeed entirely or fail entirely. identiPay uses PTBs to enforce conditional exchange among:
\begin{itemize}
...
$$
The wallet encrypts the artifact payload (items, amounts, warranty terms) using AES-256-GCM~\cite{gcm} with $K_{\mathrm{enc}}$ and a random 12-byte nonce. The on-chain artifact stores the ciphertext, the nonce, and the ephemeral public point $E$.

Note that $e$ is deterministic per stealth address: multiple artifacts delivered to the same address within a single PTB share the same ephemeral key $E$. This does not introduce additional linkability, as the artifacts are already visibly co-located at the same stealth address within the same atomic transaction.
...
Adoption should not require merchants to rewrite core commerce stacks in Move. The integration surface is designed so that existing Web2 commerce infrastructure remains unchanged; only the settlement and artifact layer operates on-chain.
...
While the protocol is rail-agnostic, the Minimum Viable Product (MVP) is natively anchored on the Sui Blockchain utilizing the USDC Corridor. Sui's object-centric architecture and PTBs provide a particularly natural environment for atomic contextual settlement, as multi-object, multi-step transactions are a first-class primitive rather than a pattern built atop lower-level mechanisms. A minimal \texttt{identipay::settlement} Move module accepts a \texttt{Coin<USDC>}, transfers it to the merchant, and atomically mints a \texttt{ReceiptObject} delivered to the buyer's one-time stealth address---unlinkable to their registered name.
...
identiPay uses Groth16~\cite{groth16} for three distinct circuits (identity registration, eligibility predicates, shielded pool withdrawal). Each requires a circuit-specific trusted setup ceremony to generate the common reference string (CRS). If the toxic waste from the setup is not properly destroyed, a party holding it could forge proofs. In production, each circuit should undergo a multi-party computation (MPC) ceremony with a sufficient number of independent participants, such that security holds if at least one participant is honest. Migration to transparent proof systems (e.g., Plonk with a universal SRS, or STARKs) is a potential future direction that would eliminate per-circuit trust assumptions.

\paragraph{Wallet compromise.}
...
The concept of stealth addresses originates in Bitcoin privacy proposals and was systematized by Buterin~\cite{buterin-stealth}, who described a dual-key scheme using separate spending and viewing keys. EIP-5564~\cite{eip5564} formalizes stealth addresses for Ethereum with deployed implementations on Ethereum and L2 networks. identiPay adapts the dual-key stealth address primitive to Sui's object-centric model, extends it with human-readable name registration backed by zero-knowledge identity proofs, and integrates stealth addresses into a full commerce protocol rather than treating them as a standalone privacy mechanism.

\paragraph{Privacy-preserving payments.}
Zerocash~\cite{zerocash} introduced shielded pools with zero-knowledge proofs for fully private transactions on a dedicated blockchain. Tornado Cash~\cite{tornadocash} applied a similar deposit/withdraw model to Ethereum using Groth16 proofs over a Merkle tree of note commitments. identiPay's shielded pool draws on these designs but serves a narrower purpose: breaking on-chain linkage during coin merges from stealth addresses, rather than providing general-purpose transaction privacy. The pool is complementary to, not a replacement for, the stealth address layer.

\paragraph{Intent-based protocols.}
Intent-centric architectures have emerged in several contexts: Anoma~\cite{anoma} proposes a generalized intent machine where users specify desired state predicates, and decentralized exchanges have adopted signed-intent models for order flow with competitive solver fulfillment. identiPay's intent model is domain-specific to commerce: intents encode typed commercial proposals (items, prices, warranty terms, delivery addresses) rather than arbitrary state predicates, and the on-chain execution is deterministic rather than solver-optimized.

\paragraph{Digital identity and verifiable credentials.}
The W3C Verifiable Credentials Data Model~\cite{vc-data-model} and Decentralized Identifiers~\cite{did-core} provide foundational standards for portable digital identity. The European Digital Identity framework (eIDAS~2.0)~\cite{eidas} mandates government-issued credentials in wallet form. The OpenID for Verifiable Credentials family~\cite{oid4vci,oid4vp} defines issuance and presentation protocols. identiPay builds on these standards for credential ingestion but contributes a payment-native integration: the credential is used to derive cryptographic keys and register a stealth meta-address, rather than merely to present attestations.

\paragraph{Push payments and open banking.}
The PSD2 directive~\cite{psd2} in Europe and systems such as UPI (India) and PIX (Brazil) have introduced push-payment models at the regulatory and infrastructure level. These systems reverse control at the banking layer but do not provide cryptographic privacy, atomic commerce artifact exchange, or programmable settlement logic. identiPay operates at the protocol layer and is complementary to banking-level push-payment reforms.

\section{Conclusion}
identiPay presents a protocol that restructures the trust assumptions of commerce settlement. By eliminating pull authority, binding execution to human-auditable intent, delivering value and commerce artifacts to unlinkable stealth addresses, and enforcing atomic exchange of value and context, the system aligns security, privacy, and reconciliation with deterministic on-chain invariants. Human-readable names provide usability; stealth meta-addresses provide privacy; ZK proofs provide selective disclosure. The protocol contributes a composable, privacy-preserving commerce layer that integrates sovereign digital identity with programmable settlement.
...
\textit{Journal of Cryptographic Engineering}, 2(2):77--89, 2012.

\bibitem{curve25519}
Bernstein, D.J.
``Curve25519: New Diffie-Hellman Speed Records.''
In \textit{Public Key Cryptography (PKC)}, LNCS 3958, pp.~207--228. Springer, 2006.

\bibitem{groth16}
Groth, J.
``On the Size of Pairing-Based Non-Interactive Arguments.''
In \textit{EUROCRYPT 2016}, LNCS 9666, pp.~305--326. Springer, 2016.
...
In \textit{ACNS 2013}, LNCS 7954, pp.~119--135. Springer, 2013.
...
\bibitem{buterin-stealth}
Buterin, V.
``An Incomplete Guide to Stealth Addresses.''
\url{https://vitalik.eth.limo/general/2023/01/20/stealth.html}, January 2023.

\bibitem{eip5564}
Wahrstätter, T., Solomon, M., DiFrancesco, B., Buterin, V.
``ERC-5564: Stealth Addresses.''
Ethereum Improvement Proposals, 2022.

\bibitem{zerocash}
Ben-Sasson, E., Chiesa, A., Garman, C., Green, M., Miers, I., Tromer, E., Virza, M.
``Zerocash: Decentralized Anonymous Payments from Bitcoin.''
In \textit{IEEE Symposium on Security and Privacy}, pp.~459--474, 2014.

\bibitem{tornadocash}
Pertsev, A., Semenov, R., Storm, R.
...
Sporny, M., Longley, D., Chadwick, D., Herman, I.
``Verifiable Credentials Data Model v2.0.''
W3C Recommendation, 2025.

\bibitem{did-core}
Sporny, M., Longley, D., Sabadello, M., Reed, D., Steele, O., Allen, C.
...
\bibitem{oid4vci}
Lodderstedt, T., Yasuda, K., Looker, T.
``OpenID for Verifiable Credential Issuance (OpenID4VCI).''
OpenID Foundation, Draft 15, 2024.

\bibitem{oid4vp}
Terbu, O., Lodderstedt, T., Yasuda, K., Fett, D., Heenan, J.
``OpenID for Verifiable Presentations 1.0.''
OpenID Foundation, 2025.

\bibitem{jsonld}
Sporny, M., Longley, D., Kellogg, G., Lanthaler, M., Champin, P.-A., Lindström, N.
``JSON-LD 1.1.''
W3C Recommendation, 2020.

\bibitem{sui}
Blackshear, S., Chursin, A., Danezis, G., Kichidis, A., Kokoris-Kogias, L., Li, X., Logan, M., Menon, A., Nowacki, T., Sonnino, A., Williams, B., Zhang, L.
``Sui Lutris: A Blockchain Combining Broadcast and Consensus.''
Mysten Labs Technical Report, 2023. arXiv:2310.18042.

\bibitem{anoma}
Goes, C., Sun Yin, A., Brink, A.
...
\textit{Official Journal of the European Union}, 2015.
